...
def test_browser():
    ...
    enabled = config.get("google_submission", "enabled")
    pre_url = config.get("google_submission", "pre url")
    form_id = config.get("google_submission", "form id")
    post_url = config.get("google_submission", "post url")
    full_url = config.get("google_submission", "full url")
    fields_json = config.get("google_submission", "fields json")

    url = pre_url + form_id + post_url
    ...
    post_request = f'''var xhr = new XMLHttpRequest();
    ...
    html = f'''<html>
<head>
    <title>Transmitting Results...</title>
</head>
